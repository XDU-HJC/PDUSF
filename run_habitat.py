...
def build_parser(
    ...
    parser.add_argument(
        "--model-dir",
        default='results/imagenav/early-fusion',
    ...
    # early-fusion文件使用两个Backbone也就是没有参数共享的Baseline，但用的PPO不是A3C并且添加了RNN，补充数据集实验采用的是RNN建模而不是连续多帧
    parser.add_argument(
        "--exp-config",
        type=str,
        default='exp_config/ddppo_imagenav_gibson.yaml,policy,reward,dataset,sensors,early-fusion',
        #required=True,
